...
class GazeModel_Teacher(nn.Module):
    def __init__(self, block=Bottleneck, layers_scene=[3, 4, 6, 3, 2], layers_face=[3, 4, 6, 3, 2]):
        ...
        super(GazeModel_Teacher, self).__init__()
    ...
    def get_deconv(self):
        ...
        deconv.append(self.deconv1)
        deconv.append(self.deconv_bn1)
        deconv.append(self.relu)
        deconv.append(self.deconv2)
        deconv.append(self.deconv_bn2)
        deconv.append(self.relu)
        deconv.append(self.deconv3)
        deconv.append(self.deconv_bn3)
        deconv.append(self.relu)
        deconv.append(self.conv4)
    ...
    def forward(self, images, head, face):
        ...
        attn_applied_scene_feat = torch.mul(attn_weights, scene_feat)    # (N, 1, 7, 7)
        scene_face_feat = torch.cat((attn_applied_scene_feat, face_feat), 1)    # torch.Size([2, 2048, 7, 7])
        ...
        encoding = self.relu(encoding)  # （N, 512, 7, 7）
        ...
        return x, encoding, encoding_inout
