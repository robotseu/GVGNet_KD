...
from code.layers.aspp import aspp_decoder
...
class MCNhead(nn.Module):
    ...
    def forward(self, xin, mid, yin, x_label=None, y_label=None, x_map=None, y_map=None, x_attn=None, y_attn=None):
        output = self.dconv(xin)
        mask = self.sconv([mid, yin])
        ...
        x_shift = dtype(np.broadcast_to(
            np.arange(fsize, dtype=np.float32), output.shape[:4]).copy()).to(devices)
        y_shift = dtype(np.broadcast_to(
            np.arange(fsize, dtype=np.float32).reshape(fsize, 1), output.shape[:4]).copy()).to(devices)

        masked_anchors = np.array(self.masked_anchors)

        w_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 0], (1, self.n_anchors, 1, 1)), output.shape[:4]).copy()).to(devices)
        h_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 1], (1, self.n_anchors, 1, 1)), output.shape[:4]).copy()).to(devices)
